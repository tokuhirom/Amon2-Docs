...
sub prepare {
    ...
    my $sth = $self->SUPER::prepare(@args) or do {
        BBS::DBI::Util::handle_error($_[1], [], $self->errstr);
    };
    $sth->{private_sql} = $_[1];
    return $sth;
}
...
sub execute {
    ...
    $self->SUPER::execute(@args) or do {
        BBS::DBI::Util::handle_error($self->{private_sql}, \@args, $self->errstr);
    };
}

sub sql { $_[0]->{private_sql} }

package BBS::DBI::Util;
use Carp::Clan qw{^(DBI::|BBS::DBI::|DBD::)};
use Data::Dumper ();
